...
async def run_level3() -> dict:
    """Run the Smith keystone PLUS the synthetic L3 cases (3 domains)."""
    import json
    from collections import defaultdict
    from app.orchestrator import evaluate_pa_case
    from tests.evals.level3_e2e.synthetic_bundle import build_pas_bundle
    from tests.evals.level3_e2e.test_l3_synthetic import L3_SYNTHETIC_CASES

    # Smith keystone (from real PDF fixture)
    cases = [
        {"id": "L3-E01-smith", "fixture": "tests/fixtures/smith_claim_bundle.json",
         "expected_outcome": "pend", "domain": "esi"},
    ]
    ...
    # First: Smith from on-disk fixture
    for c in cases:
        ...
        try:
            run = await evaluate_pa_case(bundle, run_intake_on_documents=True, case_id=c["id"])
            if run.adjudication:
            ...
            if actual == c["expected_outcome"]:
                ...
                print(f"  ✓ {c['id']:24s} [{c['domain']:12s}] outcome={actual}")
            else:
                failures.append((c["id"], f"expected {c['expected_outcome']} got {actual}"))
                by_domain[c["domain"]]["fail"] += 1
                print(f"  ✗ {c['id']:24s} [{c['domain']:12s}] expected {c['expected_outcome']} got {actual}")
        except Exception as e:
            failures.append((c["id"], str(e)))
            by_domain[c["domain"]]["fail"] += 1
            print(f"  ! {c['id']}: {e}")

    # Then: synthetic L3 cases
    for syn in L3_SYNTHETIC_CASES:
        bundle = build_pas_bundle(syn)
        try:
    ...
    total = len(cases) + len(L3_SYNTHETIC_CASES)
    return {
        "total": total,
        "passed": passes,
        "failed": len(failures),
